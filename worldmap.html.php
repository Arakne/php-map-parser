...
<script lang="js">
    ...
    L.tileLayer('http://127.0.0.1:5000/tiles/<?= $map ?>?x={x}&y={y}&z={z}', {
        maxZoom: <?= $maxZoom ?>,
    }).addTo(mymap);
    ...
    async function fetchTeleports() {
        ...
        try {
            const res = await fetch(`/markers/<?= $map ?>?bbox=${bbox}&zoom=${z}`);
    ...
    function renderGeoJSON(geojson) {
        ...
        L.geoJSON(geojson, {
            pointToLayer: (feat, latlng) => {
                ...
                const z = mymap.getZoom();
                if ((p.minZoom && z < p.minZoom) || (p.maxZoom && z > p.maxZoom)) return null;
